 ...
 */
 /* * {
  margin: 0;
  padding: 0;
  box-sizing: border-box; /* 요소의 크기를 계산할 때 padding과 border를 포함 */
/* }  */
...
.s-btn:not(.s-btn--radio:checked + label):not(.s-btn__link):not(.s-btn__unset):not(.s-btn__facebook):not(.s-btn__github):not(.s-btn__google):not(.is-selected):hover:visited:not(:active):not(:focus) {
